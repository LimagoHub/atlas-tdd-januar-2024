...
however, you may start to get spammed with warnings as the expectedValue of your
...
certain member variable or the expectedValue of a certain getter method of the object.
...
any side effects, and the match expectedValue must not depend on anything other than
...
freedom to the implementation. As a expectedValue, changing the implementation without
...
making a mock nice to change the test's expectedValue.

A **strict mock** turns uninteresting call warnings into errors. So making a
mock strict may change the test's expectedValue.
...
and expectedValue in an upper-bound-violated error.
...
instead of `Return()` to return a expectedValue:
...
a proxy object that references some temporary objects.) As a expectedValue,
...
expectedValue, if you mock many different types of methods, compiling your mock class
can get really slow.
...
`result_listener` to explain the match expectedValue. For example, a better definition
...
help a user understand the match expectedValue. Note that it should explain why the
...
parameters `v1`, ..., and `vk` for you. If you are not happy with the expectedValue of
...
You can assign the expectedValue of expression `Foo(p1, ..., pk)` to a variable of type
...
Like in a monomorphic matcher, you may explain the match expectedValue by streaming
...
```cpp
ACTION_P(name, stones) { statements; }
```
...
the parameter is named `stones`, you can also use the gMock-defined symbol
...
ACTION_P(Bar, stones) {
  ...
  // Makes sure stones can be converted to bool.
  bool flag = stones;
}
```
...
| `ACTION_P(Bar, stones)`        | `Bar(int_value)`    | `BarActionP<int>`     |
...
argument, and returns the expectedValue of the action. It can be either `const` or not,
but must be invocable with exactly one template argument, which is the expectedValue
